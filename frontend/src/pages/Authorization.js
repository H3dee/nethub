  ...
  onSubmit(event) {
    ...
    a.then((res) => {
      ...
      if (e.response.data['status'] === '401')
        await this.setState({ error_password: e.response.data['error'] });
      else this.setState({ error_password: '' });
      if (e.response.data['status'] === '418')
        await this.setState({ error_email: e.response.data['error'] });
      ...
      <div className="full-screen background centering">
        <form className="window" onSubmit={this.onSubmit}>
          ...
          <div className="input margin-bottom-20">
            <div className={`input-header ${this.state.error_email && 'error'} margin-bottom-8`}>
              e-mail
              {this.state.error_email && <span className="mini-msg">{this.state.error_email}</span>}
            </div>
            <input className="input-field" onChange={this.onChangeLogin} />
          </div>
          <div className="input margin-bottom-40">
            <div className={`input-header ${this.state.error_password && 'error'} margin-bottom-8`}>
              password
              {this.state.error_password && (
                <span className="mini-msg">{this.state.error_password}</span>
              )}
            </div>

...
class ClientTopology extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.location.state !== undefined) {
      let data = api.TopologyGet(
        this.props.location.state.request[0],
        this.props.location.state.request[1],
      );
      data
        .then((data) => {
  ...
  gridMove(event) {
    this.setState({
      grid: [
        this.state.grid[0] + event.movementX,
        this.state.grid[1] + event.movementY,
        this.state.grid[2],
      ],
    });
  }
  ...
  clickDevice(event) {
    if (event.nativeEvent.which === 1) {
      let data = api.UpdateTicket(
        event.target.getAttribute('device').split(', ')[1],
        this.props.location.state.request[0],
        this.props.location.state.request[1],
      );
      ...
      data
        .then((res) => {
          ...
          this.setState({
            devices: {
              ...this.state.devices,
              [key]: { ...this.state.devices[key], link: res.data.link },
            },
          });
        })
        .catch((res) => {
      ...
      this.setState({
        contextmenu: {
          1: {
            name: event.target.getAttribute('device').split(', ')[1],
            x: event.pageX,
            y: event.pageY,
          },
        },
      ...
      return (
        <div className="full-screen">
            ...
            >
              {Object.entries(this.state.devices).map(([key, value]) => {
                if (value.name === 'input') {
                  ...
                  return <div className={`device grab`} style={{ left: value.x, top: value.y }} />;
                } else {
                  return (
                    <a
                      onClick={this.clickDevice}
                      onContextMenu={this.clickDevice}
                      device={`${key}, ${value.vm}`}
                      target="_blank"
                      href={value.link}
          ...
          <div className="header border-bottom centering-vertically">
            ...
            <div className="title right">
              {this.state.seconds || this.state.minutes || this.state.hours
                ? `${('0' + String(this.state.hours)).slice(-2)}:${(
                    '0' + String(this.state.minutes)
                  ).slice(-2)}:${('0' + String(this.state.seconds)).slice(-2)}`
                : ''}{' '}
              {this.props.location.state.request[2]}
              <Link to="/main" className="button button-base centering margin-left-10">
                Back to main
              </Link>
              <div onClick={this.Exit} className="button button-error centering margin-left-10">
                Quit
              </div>
            </div>
          </div>
          ...
          {Object.entries(this.state.contextmenu).map(([key, value]) => {
            if (value !== undefined) {
              return (
                <div className="contextmenu" style={{ left: value.x, top: value.y }}>
                  <div
                    className="contextmenu-stage centering-vertically"
                    functional={value.name}
                    onMouseDown={this.clickContextMenu}
                  >
                    <div className="contextmenu-stage-reload" functional={value.name}></div>сброс вм
                  </div>
                </div>
              );
            }
